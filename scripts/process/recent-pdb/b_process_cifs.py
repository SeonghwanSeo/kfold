...
TOKEN_COUNT_FILTERED = 8
FEW_RESOLVED_POLYMER_FILTERED = 9
EMPTY_STRUCTURE_FILTERED = 10
INVALID_CHAIN_FILTERED = 11
...
def check_chain_count_cutoff(raw_struct: gemmi.Structure, max_chains: int) -> bool:
    ...
    return num_polymer_chains <= max_chains


def check_residue_count_cutoff(raw_struct: gemmi.Structure, max_residues: int) -> bool:
    """Returns True if the structure passes the residue count filter."""
    return all(
        len(entity.full_sequence) <= max_residues for entity in raw_struct.entities
    )


def check_total_residue_count_cutoff(
    raw_struct: gemmi.Structure, max_residues: int
) -> bool:
    """Returns True if the structure passes the residue count filter."""
...
def parse_cif(
    ...
    # Filter by total token count (naive residue count)
    if data_filter.handle_invalid_chains == "disallow":
        if not check_total_residue_count_cutoff(raw_struct, data_filter.max_tokens):
    ...
    if not cif_factory.validate_target(ref_struct):
        return FEW_RESOLVED_POLYMER_FILTERED
    ...
    if ref_struct.num_polymer_chains == 0:
        return EMPTY_STRUCTURE_FILTERED
    if not (ref_struct.num_polymer_chains <= data_filter.max_chains):
        return CHAIN_COUNT_FILTERED
    if not all(
        c.num_residues <= data_filter.max_residues
        for c in ref_struct.chains
        if c.is_polymer
    ):
        return RESIDUE_COUNT_FILTERED
    if not (ref_struct.num_tokens <= data_filter.max_tokens):
        return TOKEN_COUNT_FILTERED
...
def main():
    ...
    cif_paths = [p for p in cif_paths if "9br7" in p.name]
    ...
    print(
        f"  Less than 4 resolved residue filtered: "
        f"{results.count(FEW_RESOLVED_POLYMER_FILTERED)}"
    )
