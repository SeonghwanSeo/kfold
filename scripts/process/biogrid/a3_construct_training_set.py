"""Pack BioGRID NPZs and write manifest plus unique-protein FASTA files."""
...
def process_npz(path: pathlib.Path) -> tuple[str, bytes, dict, list[dict]]:
    ...
    for chain in ref_struct.chains:
        if not chain.ctype.is_protein or chain.entity_id in seen_entities:
            continue
        seen_entities.add(chain.entity_id)
        sequence_rows.append(
            {
                "entry_id": ref_struct.id,
                "chain_name": f"protein_{len(sequence_rows)}",
                "chain_type": "protein",
...
def write_sequences(dataset_dir: pathlib.Path, rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        if sequence not in unique_by_sequence:
            unique_by_sequence[sequence] = f"uniq_protein_{len(unique_by_sequence) + 1}"
        row["unique_id"] = unique_by_sequence[sequence]
    ...
    write_fasta(
        [(unique_id, sequence) for sequence, unique_id in unique_by_sequence.items()],
        sequence_dir / "unique_protein_sequences.fasta",
    )
    ...
    print(f"Unique protein sequences: {len(unique_by_sequence)}")
    ...
    with multiprocessing.Pool(processes=args.num_workers) as pool:
        ...
        try:
            for i, (key, value, metadata, seq_rows) in enumerate(
                tqdm(results, total=len(npz_paths), desc="Writing BioGRID LMDB"),
                start=1,
            ):
