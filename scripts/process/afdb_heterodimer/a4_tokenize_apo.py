"""Tokenize AFDB heterodimer protein apo structures from apo LMDBs."""
        ...
        with env_out.begin(write=True) as txn:
            for batch in (pbar := tqdm(dataloader, desc=f"Tokenizing {source}")):
                ...
                if max(lengths) <= BATCH_THRESHOLD:
                    ...
                    for key, tokens, length in zip(keys, combined, lengths, strict=True):
                        txn.put(key.encode("utf-8"), tokens[:, :length].tobytes())
                else:
                    for i, key in enumerate(keys):
                        ...
                        txn.put(key.encode("utf-8"), combined_i.tobytes())
