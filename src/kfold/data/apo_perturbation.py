...
class ApoPerturbation:
    ...
    def __init__(
        ...
        use_entity_random_rotation: bool = False,
        ...
        self.use_entity_random_rotation: bool = use_entity_random_rotation
    ...
    @staticmethod
    def _apply_masked_rotation(
        coords: np.ndarray, mask: np.ndarray, rotation: np.ndarray
    ) -> np.ndarray:
        """Apply a 3D rotation to masked atoms only (mask=False atoms unchanged)."""
        if coords.size == 0:
            return coords
        ...
        # Flatten for simpler math
        N, A, _ = out.shape
        flat_coords = out.reshape(N * A, 3)
        flat_mask = mask.reshape(N * A).astype(bool)

        masked_coords = flat_coords[flat_mask]  # [M, 3]
        if masked_coords.shape[0] == 0:
            return out

        center = masked_coords.mean(axis=0)  # [3]
        centered = masked_coords - center
        rotated = centered @ rotation
        flat_coords[flat_mask] = rotated + center

        return out

    def _get_or_sample_entity_rotation(
        self,
        entity_key: tuple[int, int, int],
        rng: np.random.Generator,
        cache: dict[tuple[int, int, int], np.ndarray],
    ) -> np.ndarray:
        """Return a cached entity rotation or sample a new one."""
        if not self.use_entity_random_rotation:
            return np.eye(3, dtype=np.float32)
        if entity_key in cache:
            return cache[entity_key]

        R = random_rotations_npy((), dtype=np.float32, rng=rng)
        cache[entity_key] = R
        return R
    ...
    def sample_and_augment_apo_structure(
        ...
        entity_rotations: dict[tuple[int, int, int], np.ndarray] = {}
        ...
        for chain_i in range(struct.num_chains):
            ...
            # Entity-level random rotation (applied before caching).
            R = self._get_or_sample_entity_rotation(entity_key, rng, entity_rotations)
            chain_apo_coords = self._apply_masked_rotation(
                chain_apo_coords, chain_apo_mask, R
            )
