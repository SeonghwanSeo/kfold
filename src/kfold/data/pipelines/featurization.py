...
class InputFeaturizer:
    ...
    def __init__(
        ...
        self.logger = logging.getLogger("InputFeaturizer")
    ...
    def to_folding_input(
        ...
        return self.featurize_structure(struct, rng=rng)
    ...
    def add_precomputed_embedding(
        ...
        if self.seq_embedding_dim is not None:
            ...
            pretrained_dict["sequence_embedding"] = (
                self.load_pretrained_sequence_embedding(
                    f_input,
        ...
        if self.struct_embedding_dim is not None:
            ...
            pretrained_dict["structure_embedding"] = (
                self.load_pretrained_structure_embedding(
                    f_input,
    ...
    def load_pretrained_sequence_embedding(
        ...
        for cidx in range(f_input.num_chains):
            ...
            try:
                ...
                self.logger.error(
                    f"Index error when loading sequence embedding for entity_id "
                    f"{entity_id}: {e}"
                    f"Embedding shape: {embedding_tensor.shape}, "
                    f"residue indices: {residue_indices}"
                )
                continue
        ...
        for cidx in range(f_input.num_chains):
            ...
            if residue_mask.any():
                try:
                    ...
                    self.logger.error(
                        f"Index error when loading structure embedding for entity_id "
                        f"{entity_id}: {e}"
                        f"Embedding shape: {embedding_tensor.shape}, "
                        f"residue range: {res_st}-{res_end}, "
                        f"valid residue indices: {valid_residue_indices}"
                    )
                    continue
            else:
