...
from kfold.boltz_local.modules.encoders import (
    AtomAttentionEncoder,
)
from kfold.boltz_local.modules.utils import LinearNoBias
...
class AF3InputEmbedder(BaseInputEmbedder):
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.num_pocket_contact_types: int = C.NUM_POCKET_CONTACT_TYPES

        # out projection
        s_input_dim = cfg.channel_s + self.num_res_types + self.num_pocket_contact_types
    ...
    def forward(self, f_input: FoldingInput) -> torch.Tensor:
        ...
        pocket_constraint = f_input.token.pocket_contact_type  # [L,]
        s = torch.cat(
            [
                a,
                F.one_hot(res_type, self.num_res_types).float(),
                F.one_hot(pocket_constraint, self.num_pocket_contact_types).float(),
            ],
            dim=-1,
        )
