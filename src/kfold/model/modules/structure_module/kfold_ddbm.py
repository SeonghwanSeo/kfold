...
class KFoldBridgeDiffusion(BaseStructureModule):
    ...
    class Config(BaseConfig):
        ...
        cov_xy : float, optional
            The covariance between source and target (sigma_0T in paper), by default 128.0
            (= sigma_data^2 / 2). This controls the c_skip coefficient: 0 means no
            correlation (pure exploration), sigma_data^2/2 means c_skip=0.5 at sigma_max
            (moderate guidance).
            ...
            sigma_max, balancing the skip connection between input and network output in
            the preconditioning (see Eq. 11-12, Appendix A.5).
    ...
    def _to_d_bridge(
        ...
        x_apo : torch.Tensor, optional
            Source apo structure (x_0 endpoint).
            Required if use_progressive_refinement=True.
            Shape (B, N, La, 3).
        x_holo : torch.Tensor, optional
            Target holo structure (x_T endpoint).
            Required if use_progressive_refinement=False.
            Shape (B, N, La, 3).
        guidance_weight : float, optional
        ...
        d : torch.Tensor or tuple[torch.Tensor, torch.Tensor]
            ...
            If stochastic=True: Tuple of (drift d, diffusion gt2).
                Both Shape (B, N, La, 3).
        """
        ...
        grad_pxTlxt = (x_holo_estimate - x) / (self.sigma_max**2 - sigma**2)
