...
from kfold.boltz_local.modules.transformers import (
    ConditionedTransitionBlock,
    DiffusionTransformer,
)
from kfold.boltz_local.modules.utils import (
    LinearNoBias,
...
class DiffusionModule(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pairwise_conditioner = PairwiseConditioning(
            channel_z=channel_z,
            dim_token_rel_pos_feats=channel_z,
            num_transitions=conditioning_transition_layers,
        )
        ...
        self.s_to_a_linear = nn.Sequential(
            nn.LayerNorm(2 * channel_s), LinearNoBias(2 * channel_s, 2 * channel_s)
        )
        init.final_init_(self.s_to_a_linear[1].weight)

        self.token_transformer = DiffusionTransformer(
            dim=2 * channel_s,
            dim_single_cond=2 * channel_s,
            dim_pairwise=channel_z,
    ...
    def forward(
        ...
        s, normed_fourier = self.single_conditioner(
            times=times,
            s_trunk=expand_batch(s_trunk, multiplicity),
            s_inputs=expand_batch(s_inputs, multiplicity),
        )
        ...
        mask = expand_batch(input.token.pad_mask, multiplicity)
...
class OutTokenFeatUpdate(Module):
    ...
    def __init__(
        ...
        self.norm_next = nn.LayerNorm(2 * channel_s)
        self.fourier_embed = FourierEmbedding(dim_fourier)
        self.norm_fourier = nn.LayerNorm(dim_fourier)
        self.transition_block = ConditionedTransitionBlock(
            2 * channel_s, 2 * channel_s + dim_fourier
        )
...
class AtomDiffusion(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.channel_s = score_model_args["channel_s"]
        if self.accumulate_token_repr:
            self.out_token_feat_update = OutTokenFeatUpdate(
                sigma_data=sigma_data,
                channel_s=score_model_args["channel_s"],
    ...
    def forward(
        ...
        # FIXME: here we may want to sample custom prior
