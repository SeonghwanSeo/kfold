...
def _get_minimal_slice_set(
    ...
    def reduce_edge_list(lst):
        tally = 1
        for i in range(len(lst)):
            reversed_idx = -1 * (i + 1)
            lst[reversed_idx] *= tally
            tally = lst[reversed_idx]
...
def chunk_layer(
    ...
    if _out is not None:
        reshape_fn = lambda t: t.view([-1] + list(t.shape[no_batch_dims:]))  # noqa
    ...
    for _ in range(no_chunks):
        ...
        if out is None:
            allocate = lambda t: t.new_zeros((flat_batch_dim,) + t.shape[1:])  # noqa
            ...
            def assign(d1, d2):
                for k, v in d1.items():
                    if type(v) is dict:
                        ...
                        if _add_into_out:
                            v[i : i + chunk_size] += d2[k]  # noqa
                        else:
                            v[i : i + chunk_size] = d2[k]  # noqa

            assign(out, output_chunk)
        elif out_type is tuple:
            for x1, x2 in zip(out, output_chunk):  # noqa
                if _add_into_out:
    ...
    reshape = lambda t: t.view(orig_batch_dims + t.shape[1:])  # noqa
