...
class RelativePositionEncoder(Module):
    ...
    def forward(self, f_input: FoldingInput):
        ...
        b_same_residue = torch.eq(
            f_input.token.residue_index[:, :, None],
            f_input.token.residue_index[:, None, :],
        )
        b_same_entity = torch.eq(
            f_input.token.entity_id[:, :, None], f_input.token.entity_id[:, None, :]
        )
        ...
        d_token = torch.clip(
            f_input.token.token_index[:, :, None]
            - f_input.token.token_index[:, None, :]
            + self.r_max,
            0,
            2 * self.r_max,
...
class AtomAttentionEncoder(Module):
    ...
    def forward(
        ...
        B, N, _ = f_input.atom.ref_pos.shape
        ...
        if model_cache is None or len(layer_cache) == 0:
            ...
            atom_ref_pos = f_input.atom.ref_pos
            atom_uid = f_input.atom.ref_space_uid
            atom_feats = torch.cat(
                [
                    atom_ref_pos,
                    f_input.atom.ref_charge.unsqueeze(-1),
                    f_input.atom.pad_mask.unsqueeze(-1),
                    f_input.atom.ref_element,
                    f_input.atom.ref_atom_name_chars.reshape(B, N, 4 * 64),
                ],
                dim=-1,
            )
