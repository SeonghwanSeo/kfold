...
def parse_single_file(json_or_yaml_path: str | Path, ccd: CCD) -> Query:
    ...
    # Accept the release protein_pair spelling while retaining the experiment's
    # physical-pair representation and existing multimer_sequences files.
    import copy

    original_yaml = yaml.safe_dump(input_dict)
    input_dict = copy.deepcopy(input_dict)
    entries = []
    multimers = list(input_dict.get("multimer_sequences", []))
    for entry in input_dict.get("sequences", []):
        if isinstance(entry, dict) and "protein_pair" in entry:
            from .query import Query as NativeQuery

            native = NativeQuery.from_dict(
                {"name": name, "sequences": [entry]},
                base_dir=Path(json_or_yaml_path).parent,
            ).sequences[0]
            fields = {
                "id": [":".join(pair) for pair in native.id],
                "sequence": native.sequence1 + ":" + native.sequence2,
                "modifications1": {m.residue_index: m.ccd for m in native.modifications1},
                "modifications2": {m.residue_index: m.ccd for m in native.modifications2},
            }
            for key in ("apo", "prior"):
                if getattr(native, key) is not None:
                    fields[key] = [str(p) for p in getattr(native, key)]
            if native.description is not None:
