...
def validate_affinity_system(
    *,
    ...
    if ligand_asym_id is not None:
        if asym_id is None:
            raise ValueError("Ligand selection requires token asym_id metadata.")
        asym_id = _unbatch_one("asym_id", asym_id, 1).long()
        if asym_id.shape != token_mask.shape:
            raise ValueError("asym_id and token_mask must have matching shapes.")
        selected_ligand = ligand & (asym_id[token_mask] == ligand_asym_id)
        if not selected_ligand.any():
            raise ValueError(
                f"Affinity ligand asym_id {ligand_asym_id} has no ligand tokens."
            )


def resolve_affinity_ligand_asym_id(
    ref_struct: RefStructure,
    requested_ligand_id: str | None,
) -> int:
    """Resolve a user-facing ligand chain name to one numeric asym_id."""
    ligand_chains = [
        chain for chain in ref_struct.metadata.chains if chain.ctype == C.ChainType.LIGAND
    ]
    if requested_ligand_id is None:
        if len(ligand_chains) != 1:
            names = ", ".join(chain.name for chain in ligand_chains) or "none"
            raise ValueError(
                "Affinity inference requires exactly one ligand chain unless "
                "affinity_ligand_id is specified; found: "
                f"{names}."
            )
        return ligand_chains[0].asym_id

    matches = [chain for chain in ligand_chains if chain.name == requested_ligand_id]
    if not matches:
        names = ", ".join(chain.name for chain in ligand_chains) or "none"
        raise ValueError(
            f"Affinity ligand '{requested_ligand_id}' is not a ligand chain. "
            f"Available ligand chains: {names}."
        )
    return matches[0].asym_id
...
def protein_ligand_distogram_profile(
    *,
    ...
    if ligand_asym_id is not None:
        ...
        ligand = ligand[asym_id[ligand] == ligand_asym_id]
...
def affinity_head_state_dict(
    checkpoint: object,
) -> dict[str, torch.Tensor]:
    """Extract a structure-style inference state dict from a head checkpoint."""
    if not isinstance(checkpoint, Mapping):
        raise ValueError("Affinity checkpoint must be a state dictionary mapping.")
    ...
    if not model_state:
        raise ValueError("Affinity checkpoint contains no tensor state dictionary.")
    return model_state
...
def build_per_query_affinity_inputs(
    *,
    ...
    10-token protein windows. Every token of the selected ligand is retained,
    and windows are accepted until the 256-total/200-protein budget would overflow.
    """
    ...
    crop_asym_id = asym_id[crop_indices] if asym_id is not None else None
    ...
    if ligand_asym_id is not None:
        assert crop_asym_id is not None
        ligand_mask = ligand_mask & (crop_asym_id == ligand_asym_id)
    ...
    def forward(
        ...
        *,
        ...
        return {
            ...
            "ligand_asym_id": torch.tensor(
                -1 if ligand_asym_id is None else ligand_asym_id,
                device=inputs.crop_indices.device,
            ),
        }
...
def affinity_prediction_record(
    ...
    return {
        "schema_version": "kfold_affinity_prediction_v2",
        "prediction_p_activity": float(output["p_activity"].reshape(-1)[0].item()),
        "crop_contract": AFFINITY_LIGAND_QUERY_WINDOW_CONTRACT_V2,
