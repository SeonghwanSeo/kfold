...
import kfold.data.utils.io.fasta as write_fasta
...
def run_mmseqs2_cluster(
    ...
    chain_type : str, optional
        Type of sequences: "protein", "dna", or "rna", by default "protein".
    ...
    """
    assert chain_type in {"protein", "dna", "rna"}, (
        f"Unsupported chain type: {chain_type}. Must be 'protein', 'dna', or 'rna'."
    )
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        write_fasta.write_fasta(sequences, input_fasta)
        ...
        cmd_str = (
            ...
            f"{input_fasta} {out_prefix} {tmp_dir} "
...
def run_mmseqs2_search(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        write_fasta.write_fasta(queries, query_fasta)

        target_fasta = tmp_dir / "target.fasta"
        write_fasta.write_fasta(targets, target_fasta)
        ...
        cmd_str = (
            ...
            f"-s {sensitivity} "
            f"--max-seqs {max_seqs} "
            f"--dbtype {dbtype} --search-type {search_type} "
            f"-v {verbose} "
        )
